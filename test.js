...
const { default: rbr } = require("./dist/index.js");
...
test("deve retornar uma exceção quando a extensão não tiver .br", t => {
  t.throws(
    execa("./dist/cli.js", ["test.com"]),
    / url informada deve possuir uma das seguintes extensões:/
  );
});

test("deve retornar uma exceção quando a extensão se o tamanho for menor de dois caracteres", t => {
  t.throws(
    execa("./dist/cli.js", ["a.com.br"]),
    /O Hostname deve ter no mínimo de 2 e máximo de 26 caracteres./
  );
});
...
test("deve retornar uma exceção quando a hostname terminar com hífen", t => {
  t.throws(
    execa("./dist/cli.js", ["test-.com.br"]),
    /O Hostname não deve conter hífen no ínicio ou final./
  );
});

test("deve retornar uma exceção quando o hostname conter apenas números", t => {
  t.throws(
    execa("./dist/cli.js", ["32132123.com.br"]),
    /O Hostname não deve conter apenas números./
  );
});

test("deve retornar uma exceção quando o hostname conter caracteres inválidos", t => {
  t.throws(
    execa("./dist/cli.js", ["te$t.com.br"]),
...
test("deve retornar uma exceção quando a url é inválida", t => {
  t.throws(execa("./dist/cli.js"), /Por favor, digite uma url válida./);
});
